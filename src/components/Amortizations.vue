        ...
        <div class="container">
            ...
            <div class="amortizations-options d-flex flex-column">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" id="annual" value="Annual" v-model="selectedAmortizationType" />
                    <label class="form-check-label" for="annual">Annual amortization</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" id="number" value="Number" v-model="selectedAmortizationType" />
                    <label class="form-check-label" for="number">Specific number of amortizations</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" id="custom" value="Custom" v-model="selectedAmortizationType" />
            ...
            <div v-if="selectedAmortizationType == 'Annual'" class="annual-amortization-body mt-4">
                <input v-model="amortizationValue" class="form-control text-center" id="inputEmail3" placeholder="Annual amount">
            </div>
            <div v-if="selectedAmortizationType == 'Number'" class="number-amortization-body mt-4">
                <input v-model="amortizationValue" class="form-control text-center " id="inputEmail3" placeholder="Payment amount">
                <div class="row mt-3">
            ...
            <div class="mt-5" v-if="amortizationValue != ''">
                <p class="text-center">You will end up paying the bank <b>{{ milesAndCommaFormat(parseFloat(totalAmountAmortizing)) }}€</b></p>
                <p class="text-center">A total of <b>{{ milesAndCommaFormat((totalAmountAmortizing-mortgageValues.amount)) }}€</b>
                    in interest (<b>{{ (((totalAmountAmortizing / mortgageValues.amount) - 1) * 100).toFixed(2) }}%</b>)
                </p>
                <p class="text-center">Total interest saved: <span class="text-success">{{ milesAndCommaFormat((mortgageValues.totalToPay - totalAmountAmortizing))}}€</span></p>
                <p class="text-center">Months reduced from:</p> 
                <p>{{ yearsFromMonths(mortgageValues.years * 12) }} ➡️ {{ yearsFromMonths(this.totalMonthsAmortizing) }}</p>
...
export default {
  ...
  methods: {
    ...
    yearsFromMonths(months) {
        let years_part = Math.floor(months / 12) + " years"
        let months_part = " and " + months % 12 + " months"
    ...
    updateToTalAmountAmortizing() {
        let totalAmountYearAmmortizationData = getTotalAmountYearAmmortization(
